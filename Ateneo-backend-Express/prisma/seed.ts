...
async function main() {
  ...
  const professors = await prisma.professor.createMany({
    data: [
      ...
      { id: 'prof2', firstName: 'Luis', lastName: 'Martínez', email: 'luis.martinez@ateneo.com', password: 'pass2', emailActivated: true },
      { id: 'prof3', firstName: 'María', lastName: 'Pérez', email: 'maria.perez@ateneo.com', password: 'pass3', emailActivated: true },
      { id: 'prof4', firstName: 'Carlos', lastName: 'López', email: 'carlos.lopez@ateneo.com', password: 'pass4', emailActivated: true },
    ]
  });
  ...
  const subjectsData = [
    { id: 'subj1', name: 'Matemática I', academicYear: 2024, institution: 'Instituto A', degree: 'Lic. Matemática', professorId: 'prof1' },
  ...
  for (let i = 0; i < 4; i++) { // solo para subj1, subj3, subj5, subj7
    for (let j = 0; j < 2; j++) {
...
if (args.includes('--reset')) {
  resetDatabase().then(main).catch(console.error).finally(async () => {
